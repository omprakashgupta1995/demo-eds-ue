...
export default function decorate(block) {
  // UE RE-RENDER PREVENTION: Stop if already decorated
  if (block.querySelector('.fund-stats-container')) return;

  // Find the true inner container (handling UE wrappers)
  let container = block.firstElementChild?.firstElementChild;
  if (!container) container = block;
  
  if (!container.classList.contains('fund-card')) {
    container.classList.add('fund-card');
  }
  ...
  if (planOptions.length === 0) {
    console.warn('Fund Details: No plan data found. Awaiting author input.');
    return; 
  }
  ...
  const durationOptions = (firstPlanData && Object.keys(firstPlanData.returns).length > 0) 
    ...
    : ['Since 1 year', 'Since 3 year', 'Since 5 year'];
  ...
  if (!btnContainer) {
    ...
    if (link) {
      ...
      btnContainer = link.closest('p') || link;
      if (btnContainer.tagName === 'P') btnContainer.classList.add('button-container');
    }
  }
  ...
  // --- 5. Clean up metadata & Style Footer ---
  if (btnContainer && btnContainer.tagName === 'P') {
    btnContainer.classList.add('fund-action');
  ...
  // Intelligently find the footer by looking for the user icon
  const allPs = Array.from(container.querySelectorAll('p'));
  const footerP = allPs.find(p => p.querySelector('img[data-icon-name="user-icon"]') || p.textContent.includes('invested in this fund'));
  if (footerP) {
    footerP.classList.add('fund-footer');
  }
}