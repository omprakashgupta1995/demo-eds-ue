...
function createLabel(fd) {
  ...
  const isMandatory =
    fd.Mandatory &&
    (fd.Mandatory.toString().toLowerCase() === "true" ||
      fd.Mandatory.toString().toLowerCase() === "x");
  if (isMandatory) {
...
const createConfirmation = (fd, form) => {
  if (fd.Value) {
    form.dataset.confirmation = new URL(
      fd.Value,
...
const createFieldset = (fd) => {
  ...
  setCommonAttributes(field, fd);
  field.name = fd.Name || ""; // Required for grouping logic
  if (fd.Label) {
